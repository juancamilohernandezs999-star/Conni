...
def topbar() -> None:
    ...
    with st.container(key="app_topbar"):
        brand, context = st.columns([0.62, 0.38], vertical_alignment="center", gap="small")
        with brand:
            st.markdown(
                f"""
                <div class="brand-lockup" title="Colsubsidio · Finanzas Corporativas">
        ...
        with context:
            home, metadata = st.columns([0.28, 0.72], vertical_alignment="center", gap="small")
            home.button(
                "⌂ Inicio",
                key="topbar_home_button",
                width="stretch",
                help="Volver al inicio sin retirar el archivo cargado",
                on_click=go_to,
                args=("Inicio",),
            )
            metadata.markdown(
...
def kpi_cards(items: list[tuple[str, str, str, str, str]]) -> None:
    cards = "".join(
        ...
        f'<div class="kpi-content"><div class="kpi-label">{label}</div>'
...
def dashboard_heading(number: str, title: str, copy: str) -> None:
    heading, back = st.columns([0.78, 0.22], gap="medium")
    with heading:
        st.markdown(
...
def render_plant_dashboard(data: ConniData) -> None:
    ...
    kpi_cards(
        [
            ...
            ("Pasivo depurado", spanish_number(metrics["vacation_days_net"]), "Días después de solicitudes", COLORS["bright"], "🗓️"),
        ]
    )
    ...
    if section == "Resumen gerencial":
        ...
        left, middle, right = st.columns([1.25, 1.05, 0.8], gap="medium")
        if not gender.empty:
            fig = px.bar(
                gender,
                x="Área",
                ...
                color="Género",
                barmode="group",
                title="Género por gerencia",
                color_discrete_sequence=[COLORS["cyan"], COLORS["blue"], COLORS["muted"]],
            )
            fig.update_xaxes(tickangle=-18)
            left.plotly_chart(chart_style(fig, 350), width="stretch")
        vacancy_area = area.melt(
            id_vars="Área",
        ...
        fig = px.bar(
            ...
            orientation="h",
            barmode="group",
            title="Vacantes y ONC por gerencia",
            color_discrete_map={"Vacantes": COLORS["yellow"], "ONC": COLORS["red"]},
        )
        middle.plotly_chart(chart_style(fig, 350), width="stretch")
        fig = px.pie(
            composition,
            names="Estado",
            values="Posiciones",
            hole=.58,
            title="Composición de la planta",
            color="Estado",
            color_discrete_map={"Ocupada": COLORS["blue"], "Vacantes": COLORS["yellow"], "ONC": COLORS["red"]},
        )
        fig.update_traces(textposition="inside", textinfo="percent+value")
        right.plotly_chart(chart_style(fig, 350), width="stretch")
