...
def socio_metrics(socio: pd.DataFrame) -> dict[str, float | int | str]:
    ...
    return {
        ...
        "average_age": float(age.mean()) if age.notna().any() else np.nan,
        "min_age": float(age.min()) if age.notna().any() else np.nan,
        "max_age": float(age.max()) if age.notna().any() else np.nan,
